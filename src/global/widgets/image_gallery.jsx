    ...
    return (
        <div className="flex flex-col items-center">
            <img
                ...
                className="w-full h-auto max-w-lg rounded-lg shadow-md mb-4"
            />
            <div className="relative">
                {/* the blue box arround the small images in the gallery */}
                <div
                    ref={blueBoxRef}
                    className="absolute  border-4 border-indigo-500 rounded-md"
                    style={{
                ...
                <div className="grid grid-cols-5 gap-4">
                    {images.map((image, index) => (
                        <img
                            ...
                            src={image}
                            alt={`Product ${index + 1}`}
                            className={`cursor-pointer ${index === currentImage ? 'opacity-100 ' : 'opacity-50'
                                }
                                ${initialize && index == currentImage ? ' border-4  border-indigo-500 rounded-md' : ''}

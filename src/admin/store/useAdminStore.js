...
import { adminLogin, checkUserForPassReset, confirmPinCode } from '../controller/adminController';
import {getCookie, setCookie} from '../../useCookie'
const useAdminStore = create((set) => ({
    isAdmin: getCookie('jwt_admin')||false,
    passwordReset: false,
    pinCode: null,
    setPasswordReset: (state) => { set({ passwordReset: state }) },
    error: null,
    setError: (error) => { set({ error: error }) },
    handleLogin: async (formData) => {


        await adminLogin(formData).then((jwt_admin) => {
    ...
    handleConfirmPincode: async (enteredPinCode) => {
        ...
        await confirmPinCode(enteredPinCode).then((res) => {
